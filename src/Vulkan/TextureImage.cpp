...
{
    vkDestroyImage(m_ctx->GetDevice(), m_textureImage, nullptr);
    vkDestroySampler(m_ctx->GetDevice(), m_sampler, nullptr);
    vkFreeMemory(m_ctx->GetDevice(), m_memory, nullptr);
}

VkImage TextureImage::GetImage() const { return m_textureImage; };
...
void TextureImage::createTextureImage(const std::filesystem::path& path)
{
    ...
    if (vkCreateImage(m_ctx->GetDevice(), &imageInfo, nullptr, &m_textureImage) != VK_SUCCESS)
        throw std::runtime_error("failed to create image!");

    VkMemoryRequirements memRequirements;
    vkGetImageMemoryRequirements(m_ctx->GetDevice(), m_textureImage, &memRequirements);
    ...
    vkBindImageMemory(m_ctx->GetDevice(), m_textureImage, m_memory, 0);
