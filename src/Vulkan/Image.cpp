...
Image::Image(const std::shared_ptr<VulkanContext>& ctx)
    : m_ctx(ctx)
{
    createSampler();
}
...
Image::~Image()
{
    vkDestroyImage(m_ctx->GetDevice(), m_image, nullptr);
    vkDestroySampler(m_ctx->GetDevice(), m_sampler, nullptr);
    vkFreeMemory(m_ctx->GetDevice(), m_memory, nullptr);
}

VkImage Image::GetImage() const { return m_image; }
VkSampler Image::GetSampler() const { return m_sampler; }
...
void Image::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties)
{
    ...
    if (vkCreateImage(m_ctx->GetDevice(), &imageInfo, nullptr, &m_image) != VK_SUCCESS)
        throw std::runtime_error("failed to create image!");

    VkMemoryRequirements memRequirements;
    vkGetImageMemoryRequirements(m_ctx->GetDevice(), m_image, &memRequirements);

    VkMemoryAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    allocInfo.allocationSize = memRequirements.size;
    allocInfo.memoryTypeIndex = findMemoryType(m_ctx->GetPhysicalDevice(), memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    if (vkAllocateMemory(m_ctx->GetDevice(), &allocInfo, nullptr, &m_memory) != VK_SUCCESS)
        throw std::runtime_error("failed to allocate image memory!");

    vkBindImageMemory(m_ctx->GetDevice(), m_image, m_memory, 0);
}
...
void Image::createSampler()
{
    ...
    samplerInfo.maxAnisotropy = m_ctx->GetGpuProperties().limits.maxSamplerAnisotropy;
